...
if isempty(opts.train), opts.train = imdb.train_set ; end
if isempty(opts.val), opts.val = imdb.val_set ; end
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    if strcmp(mode, 'train')
      ...
      recid                         = net.getVarIndex('regr') ;
      net.vars(recid).precious      = true;
  ...
  if (~mod(fix((t-1)/params.batchSize), 10))
      ...
      rec_      = net.vars(recid).value(:,:,:,1);
      means_    = means_(:,:,:,1);
      
      if strcmp(params.method, 'residual')
          labels_   = data_ - labels_;
          rec_      = data_ - rec_;
      end
 
      labels_   = bsxfun(@plus, labels_,    means_);
      data_     = bsxfun(@plus, data_,      means_);
      rec_      = bsxfun(@plus, rec_,       means_);
      ...
      colormap gray;
        ...
        data_     = complex(data_(:,:,1:end/2), data_(:,:,end/2+1:end));
        
        data_plot = ifftshift(ifftshift(ifft2(fftshift(fftshift(sum(data_,3),1),2)),2),1);

        if strcmp(params.method, 'residual')
          labels_plot   = abs(data_plot - complex(labels_(:,:,1:end/2,:), labels_(:,:,end/2+1:end,:)));
          rec_plot      = abs(data_plot - complex(rec_(:,:,1:end/2,:), rec_(:,:,end/2+1:end,:)));
        end
      
        
    labels_plot = ssos(complex(labels_(:,:,1:end/2,:), labels_(:,:,end/2+1:end,:)),3);
    data_plot = ssos(data_,3);
    
    rec_plot = ssos(complex(rec_(:,:,1:end/2,:), rec_(:,:,end/2+1:end,:)),3);
    
    subplot(221); imagesc(labels_plot, [0 max(labels_plot(:))]);        axis image off;     title('ground truth'); 
    subplot(222); imagesc(abs(data_plot), [0 max(labels_plot(:))]);        axis image off;     title('input'); 
    subplot(223); imagesc(rec_plot, [0 max(labels_plot(:))]);        axis image off;     title('recon'); 
    subplot(224); imagesc(rec_plot, [0 max(labels_plot(:))*0.2]);        axis image off;     title('recon'); 
